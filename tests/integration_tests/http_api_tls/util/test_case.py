...
def create_changefeed(sink_uri):
    url = f"{BASE_URL1}/changefeeds"
    # create changefeed
    for i in range(1, 5):
        data = {
            "changefeed_id": f"changefeed-test{str(i)}",
            "sink_uri": "blackhole://",
...
def list_changefeed():
    # test state: all
    url = f"{BASE_URL0}/changefeeds?state=all"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok

    # test state: normal
    url = f"{BASE_URL0}/changefeeds?state=normal"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok
    data = resp.json()
    ...
    # test state: stopped
    url = f"{BASE_URL0}/changefeeds?state=stopped"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok
    data = resp.json()
    for changefeed in data:
...
def get_changefeed():
    ...
    url = f"{BASE_URL0}/changefeeds/changefeed-test1"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok

    # test get changefeed failed
    url = f"{BASE_URL0}/changefeeds/changefeed-not-exists"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.bad_request
    data = resp.json()
...
def pause_changefeed():
    # pause changefeed
    url = f"{BASE_URL0}/changefeeds/changefeed-test2/pause"
    ...
    url = f"{BASE_URL0}/changefeeds/changefeed-test2"
    for _ in range(RETRY_TIME):
        resp = rq.get(url, cert=CERT, verify=VERIFY)
        assert resp.status_code == rq.codes.ok
        data = resp.json()
    ...
    url = f"{BASE_URL0}/changefeeds/changefeed-not-exists/pause"
    resp = rq.post(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.bad_request
    data = resp.json()
    assert data["error_code"] == "CDC:ErrChangeFeedNotExists"
...
def resume_changefeed():
    # resume changefeed
    url = f"{BASE_URL1}/changefeeds/changefeed-test2/resume"
    resp = rq.post(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.accepted

    # check if resume changefeed success
    url = f"{BASE_URL1}/changefeeds/changefeed-test2"
    for _ in range(RETRY_TIME):
        resp = rq.get(url, cert=CERT, verify=VERIFY)
        assert resp.status_code == rq.codes.ok
        data = resp.json()
    ...
    url = f"{BASE_URL0}/changefeeds/changefeed-not-exists/resume"
    resp = rq.post(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.bad_request
    data = resp.json()
    assert data["error_code"] == "CDC:ErrChangeFeedNotExists"
...
def remove_changefeed():
    # remove changefeed
    url = f"{BASE_URL0}/changefeeds/changefeed-test3"
...
def rebalance_table():
    # rebalance_table
    url = f"{BASE_URL0}/changefeeds/changefeed-test1/tables/rebalance_table"
    resp = rq.post(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.accepted
...
def move_table():
    # move table
    url = f"{BASE_URL0}/changefeeds/changefeed-test1/tables/move_table"
...
def get_processor():
    ...
    base_url = f"{BASE_URL0}/processors"
    resp = rq.get(base_url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok
    data = resp.json()[0]
    url = f"{base_url}/" + data["changefeed_id"] + "/" + data["capture_id"]
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    ...
    url = f"{base_url}/" + data["changefeed_id"] + "/" + "non-exist-capture-id"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.bad_request
...
def verify_table():
    ...
    url = f"{BASE_URL0_V2}/verify_table"
    data = json.dumps({
...
def create_changefeed_v2():
    url = f"{BASE_URL1_V2}/changefeeds"
    # create changefeed 1
    data = {
...
def unsafe_apis():
    url = f"{BASE_URL1_V2}/unsafe/metadata"
    resp = rq.get(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok
    ...
    url = f"{BASE_URL1_V2}/unsafe/service_gc_safepoint"
    data = {
        "pd_addrs": [TLS_PD_ADDR],
        "ca_path":CA_PEM_PATH,
        "cert_path":CLIENT_PEM_PATH,
    ...
    # create changefeed fail because sink_uri is invalid
    data = json.dumps({})
    url = f"{BASE_URL1_V2}/unsafe/resolve_lock"
    headers = {"Content-Type": "application/json"}
    resp = rq.post(url, data=data, headers=headers, cert=CERT, verify=VERIFY)
    print("status code", resp.status_code)
...
def delete_changefeed_v2():
    # remove changefeed
    url = f"{BASE_URL0_V2}/changefeeds/changefeed-test4"
    resp = rq.delete(url, cert=CERT, verify=VERIFY)
    assert resp.status_code == rq.codes.ok

    # check if remove changefeed success
    url = f"{BASE_URL0}/changefeeds/changefeed-test4"
    for _ in range(RETRY_TIME):
        resp = rq.get(url, cert=CERT, verify=VERIFY)
        if resp.status_code == rq.codes.bad_request:
            break
    ...
    url = f"{BASE_URL0_V2}/changefeeds/changefeed-not-exists"
    resp = rq.delete(url, cert=CERT, verify=VERIFY)
    assert (resp.status_code == rq.codes.ok)
...
if __name__ == "__main__":
    ...
    CLIENT_PEM_PATH = f'{CERTIFICATE_PATH}/client.pem'
    CLIENT_KEY_PEM_PATH = f'{CERTIFICATE_PATH}/client-key.pem'
    CA_PEM_PATH = f'{CERTIFICATE_PATH}/ca.pem'
